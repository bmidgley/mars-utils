    ...
    def do_GET(self):
        ...
        if self.path == f'/{secret}.html':
            ...
            with tempfile.TemporaryDirectory() as path:
                ...
                self.end_headers()
                self.wfile.write(payload)
        elif self.path == f'/{secret}' or self.path == f'/{secret}/' or self.path == f'/{secret}/index.html':
    ...
    if 'type' in message:
        if message['type'] == 'nodeinfo':
            ...
            if lat != 0 and lon != 0:
                ...
                add_time(station_name, message['timestamp'])
                for measure in ['battery', 'temperature', 'relative_humidity', 'voltage', 'current', 'time', 'hardware', 'node_type']:
